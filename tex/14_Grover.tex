...
\draw[thick,->] (0:0) -- (240:5) node[right=1pt] {$WV_f\ket{+}$};
...
\draw[very thick,->] (0,0) -- (5,0) node[right=1pt] {$\ket{f}$};
\draw[very thick,->] (0,0) -- (0,5) node[right=1pt] {$\ket{s}$};
\draw[thick,->] (0:0) -- (20:5) node[right=1pt] {$\ket{+}$};
\filldraw[thick,fill=red] (0,0) -- (2,0) arc[start angle=0, end angle=20, radius=2] node[pos=0.5,right=1pt] {$\theta$};
\draw[thick,->] (0:0) -- (-20:5) node[right=1pt] {$V_f\ket{+}$};
\filldraw[thick,fill=red] (0,0) -- (2,0) arc[start angle=0, end angle=-20, radius=2] node[pos=0.5,right=1pt] {$\theta$};
\draw[thick] (0:0) -- (110:5);
\draw[thick] (0:0) -- (-70:5);
\draw[thick] (0:0) -- (200:5);
\draw[thick,->] (0:0) -- (240:5) node[right=1pt] {$WV_f\ket{+}$};
\filldraw[thick,fill=red] (0,0) -- (200:2) arc[start angle=200, end angle=240, radius=2] node[pos=0.5,left=1pt] {$2\theta$};
\draw[thick,->] (0:0) -- (60:5) node[right=1pt] {$-WV_f\ket{+}$};
...
\item $O^*(2^{n/3})$ algorithm for subset sum [Brassard et al.\ 1997].
\item $O(1.728^{n})$ algorithm for Travelling salesman problem\\
{[Ambainis et al.\ 2019]}.
\item $O(1.914^{n})$ algorithm for graph coloring problem\\
...
\end{align*}
For any ``distance'' function $D$ for $\left\{\ket{a}\in\mathbb{C}^N\mid \braket{a|a} = 1\right\}$,
\begin{align*}
...
Let
\begin{align*}
D(\ket{a}, \ket{b}):= \arccos \left|\braket{a|b}\right|
\end{align*}
...
\Longleftrightarrow\quad& \arccos(|\braket{c|a}|) \le \theta_{ab}+\theta_{bc}\\
...
\begin{align*}
&\frac{\pi}2\le \left(\arccos\left(\sqrt{p_{\mathsf{succ}}}\right)+2k\theta+ \theta\right)\\
